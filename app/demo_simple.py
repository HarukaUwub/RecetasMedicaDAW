#!/usr/bin/env python3
"""
Script de Demostración Simplificado - Integración de Aplicaciones
Versión compatible con Windows que evita problemas de codificación.
"""
...
def demo_xml_validation():
    ...
    print_header("DEMOSTRACION: VALIDACION XML")
    ...
    try:
        ...
        if es_valido:
            print("[OK] XML valido segun esquema XSD")
        else:
            print(f"[ERROR] XML invalido: {errores}")
        
        print_step("3", "Parseando XML...")
        datos_parseados = parse_xml_paciente(xml_bytes)
        
        if datos_parseados:
            print("[OK] XML parseado exitosamente:")
            for campo, valor in datos_parseados.items():
                if campo != "checksum":
                    print(f"   - {campo}: {valor}")
        else:
            print("[ERROR] Error parseando XML")
        
        return True
        
    except Exception as e:
        print(f"[ERROR] Error en demostracion XML: {e}")
        return False

def demo_drive_operations():
    """Demuestra las operaciones con Google Drive."""
    print_header("DEMOSTRACION: OPERACIONES GOOGLE DRIVE")
    ...
    try:
        ...
        if service:
            print("[OK] Conectado a Google Drive")
        else:
            print("[ADVERTENCIA] Drive no disponible, usando modo local")
        ...
        if folder_info:
            if isinstance(folder_info, tuple):
                ...
                print(f"[OK] Carpetas configuradas:")
                ...
                print(f"[OK] Carpeta principal: {folder_info}")
        else:
            print("[ADVERTENCIA] No se pudieron configurar carpetas")
        ...
        print(f"[ERROR] Error en demostracion Drive: {e}")
        return False

def demo_error_handling():
    """Demuestra el manejo de errores."""
    print_header("DEMOSTRACION: MANEJO DE ERRORES")
    
    try:
        from paciente_xml_utils import validar_xml_con_xsd
        
        print_step("1", "Probando XML invalido...")
...
<paciente>
    ...
    <genero>X</genero>
        ...
        print(f"[ERROR] Error en demostracion de errores: {e}")
        return False

def demo_web_app():
    """Demuestra la funcionalidad de la aplicación web."""
    print_header("DEMOSTRACION: APLICACION WEB")
    
    try:
        from web_app_simulator import WebAppSimulator
        
        print_step("1", "Inicializando aplicacion web simulada...")
        app = WebAppSimulator()
        ...
        if resultado["success"]:
            print(f"[OK] Paciente agregado exitosamente:")
        ...
        print_step("3", "Listando pacientes en BD Web...")
        ...
        print(f"[ERROR] Error en demostracion web: {e}")
        return False

def demo_local_app():
    """Demuestra la funcionalidad de la aplicación local."""
    print_header("DEMOSTRACION: APLICACION LOCAL")
    ...
    try:
        ...
        print_step("2", "Obteniendo estadisticas iniciales...")
        ...
        print_step("3", "Ejecutando sincronizacion...")
        ...
        print(f"[ERROR] Error en demostracion local: {e}")
        return False

def main():
    """Función principal de demostración."""
    print_header("DEMOSTRACION COMPLETA - INTEGRACION DE APLICACIONES")
    print("Este script demuestra el flujo completo de sincronizacion")
    print("de pacientes entre aplicaciones usando Google Drive como puente.")
    
    demos = [
        ("Validacion XML", demo_xml_validation),
        ("Operaciones Drive", demo_drive_operations),
        ("Manejo de Errores", demo_error_handling),
        ("Aplicacion Web", demo_web_app),
        ("Aplicacion Local", demo_local_app),
    ]
    ...
    # Resumen final
    print_header("RESUMEN DE DEMOSTRACION")
    ...
    for nombre, resultado in resultados.items():
        estado = "[OK] Exitoso" if resultado else "[ERROR] Fallo"
        print(f"   - {nombre}: {estado}")
    
    if exitosos == total:
        print("\n[EXITO] Todas las demostraciones completadas exitosamente!")
        print("\nEl sistema de integracion esta funcionando correctamente.")
        print("Puedes usar:")
        print("   - web_app_simulator.py para simular la aplicacion web")
        print("   - main_with_sync.py para la aplicacion de escritorio")
        print("   - sync_pacientes.py para sincronizacion manual")
    else:
