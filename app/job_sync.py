...
import shutil
import time
from .xml_utils import validar_receta_xml, parse_xml_receta
from .database import Session, insert_receta

# --- Configuración de Logging ---
# Creamos un logger específico para el job para no mezclarlo con otros logs.
log_dir = os.path.join(os.path.dirname(__file__), '..', 'logs') # Sigue apuntando a la carpeta logs/ en la raíz
...
class SyncJob:
    ...
    def __init__(self, base_path):
        ...
        self.xsd_path = os.path.join(os.path.dirname(__file__), 'receta.xsd') # Sigue funcionando porque receta.xsd está en app/
    ...
    def _procesar_archivo(self, filename):
        ...
        try:
            ...
            datos_receta = parse_xml_receta(xml_content)
