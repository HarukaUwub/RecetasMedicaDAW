...
OUT_DIR = os.path.join(ROOT, "data", "medicos_validos")
XSD_PATH = os.path.join(os.path.join(os.path.dirname(__file__), ".."), "xsd", "medico.xsd")
DRIVE_FOLDER = os.getenv("DRIVE_EXPORT_FOLDER_MEDICOS_ID", "15Mk3wL0S2vw6G8YnoZaePHJH4UbcC7z_").strip()
...
def build_medico_xml(m: MedicoWeb) -> etree._Element:
    root = etree.Element("medico", id=str(m.id) if m.id is not None else "")
    def add(tag, text):
        if text is None:
            return
...
def export_all():
    ...
    try:
        ...
        if not medicos:
            logger.warning("No hay médicos para exportar.")
            return {"exported": 0, "details": []}
        count = 0
        results = []
        for m in medicos:
            try:
                root = build_medico_xml(m)
                
                filename = f"medico_{m.id}.xml"
                ...
                valid, errors = validate_xml_against_xsd(out_path, "medico.xsd")
                if valid:
                    uploaded = False
                ...
                logger.error(f"[EXPORT] Error procesando medico {m.id}: {e}", exc_info=True)
                results.append({"file": f"medico_{m.id}.xml", "error": str(e), "ok": False})
        return {"exported": count, "total": len(medicos), "details": results}
    except Exception as e:
